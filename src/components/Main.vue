...
const addColumnAsDefaultToSvd = (column: FileSpecificationColumn) => {
  const getDefaultUnit = (col: FileSpecificationColumn) => {
    if (col.label === 'state' || col.label === 'fault_code') {
      return col.dataType.units.find(unit => unit.name === 'Decode') || col.unit
    }
    if (col.label === 'roll' || col.label === 'pitch') {
      return col.dataType.units.find(unit => unit.name === 'Degree') || col.unit
    }
    if (col.dataType.name === 'speed') {
      return col.dataType.units.find(unit => unit.symbol === 'km/h') || col.unit
    }
    return col.unit
  }
...
const entriesPerSecondLabel = computed(() => {
  const data = logFileData.value
  if (!data || data.seriesList.length === 0) {
    return null
  }

  const entries = data.seriesList[0].data.length
  const durationSeconds = logFileDataHelper.value.endTime - logFileDataHelper.value.startTime

  if (!Number.isFinite(durationSeconds) || durationSeconds <= 0) {
    return null
  }

  const rate = entries / durationSeconds
  if (!Number.isFinite(rate)) {
    return null
  }

  return `${rate.toFixed(1)} data/s`
})
              ...
              <a-card style="width: 100%">
                <template #title>
                  ...
                  <div class="ant-card-meta-description" style="padding-top: 0px; font-weight: 400; font-size: 14px; margin-top: -8px;">
                    Open a VESC CSV log file or Floaty JSON log file
                  </div>
                </template>
                <a-space direction="vertical" style="width: 100%">
                  ...
                  <a-typography-text v-else type="secondary">
                    ...
                    <template v-if="entriesPerSecondLabel">
                      ({{ entriesPerSecondLabel }})
                    </template>
                  </a-typography-text>
                </a-space>
              </a-card>
